...
(function () {
    ...
    var settings = {
        ...
        domainsList: document.querySelector('#domainsList'),
    ...
    function saveOptions() {
        var domainsList = [], pattern;
        try {
            domainsList = settings.domainsList.value.replace(/\s/g,'').split(',');
            domainsList.some(el => (pattern = el) && !(new RegExp(el, "i")));
        } catch (er) {
            alert([pattern ? `Faild to create new Regexp using pattern: ${pattern}` : er, 'Settings are not saved.'].joun('\n'));
            return;
        }
    ...
    function getUserDefinedSettings() {
        ...
        chrome.storage.sync.get(['tab', 'optionsSettings'], function (items) {
            ...
            settings.domainsList.value = (items.settings.domainsList || []).join(",\n");
